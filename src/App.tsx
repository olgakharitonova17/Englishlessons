...
type View = { page: 'home' } | { page: 'course'; course: Course } | { page: 'module'; course: Course; module: number } | { page: 'lesson'; course: Course; module: number; lesson: string }
...
function Breadcrumbs({ view, setView }: { view: Exclude<View, { page: 'home' }>; setView: (view: View) => void }) {
  return <nav className="breadcrumbs" aria-label="Breadcrumb"><button onClick={() => setView({ page: 'home' })}>Courses</button><span>›</span>{view.page !== 'course' && <><button onClick={() => setView({ page: 'course', course: view.course })}>{view.course.title}</button><span>›</span></>}{view.page === 'lesson' && <><button onClick={() => setView({ page: 'module', course: view.course, module: view.module })}>Module {view.module}</button><span>›</span></>}<strong>{view.page === 'course' ? view.course.title : view.page === 'module' ? `Module ${view.module}` : view.lesson}</strong></nav>
}
...
function CoursePage({ course, setView }: { course: Course; setView: (view: View) => void }) {
  return <main className="inner-page"><Breadcrumbs view={{ page: 'course', course }} setView={setView}/><PageIntro kicker={course.level} title={course.title} text="Choose a module and continue your English journey." accent={course.accent}/><section className="content-section"><div className="section-heading"><div><span className="eyebrow">Course content</span><h2>Modules</h2></div><p>{course.modules} learning modules</p></div><div className="module-grid">{Array.from({ length: course.modules }, (_, i) => i + 1).map(module => <button className="module-card" key={module} onClick={() => setView({ page: 'module', course, module })}><span className="module-number">{String(module).padStart(2, '0')}</span><span><small>{module === course.modules ? 'Final module' : 'Learning module'}</small><strong>Module {module}</strong><em>5 lessons</em></span><i><ArrowIcon/></i></button>)}</div></section></main>
}
function ModulePage({ course, module, setView }: { course: Course; module: number; setView: (view: View) => void }) {
  return <main className="inner-page"><Breadcrumbs view={{ page: 'module', course, module }} setView={setView}/><PageIntro kicker={course.title} title={`Module ${module}`} text="Work through the lessons, then check your progress." accent={course.accent}/><section className="content-section"><div className="section-heading"><div><span className="eyebrow">Module content</span><h2>Lessons</h2></div><p>Learn at your own pace</p></div><div className="lesson-list">{lessons.map((lesson, index) => <button className="lesson-card" key={lesson} onClick={() => setView({ page: 'lesson', course, module, lesson })}><span className={`lesson-icon lesson-icon--${index}`} aria-hidden="true">{index < 3 ? index + 1 : index === 3 ? '↻' : '✓'}</span><span><small>{index < 3 ? 'Core lesson' : index === 3 ? 'Review everything' : 'Check your knowledge'}</small><strong>{lesson}</strong></span><i><ArrowIcon/></i></button>)}</div></section></main>
}
function LessonPage({ course, module, lesson, setView }: { course: Course; module: number; lesson: string; setView: (view: View) => void }) {
  return <main className="inner-page"><Breadcrumbs view={{ page: 'lesson', course, module, lesson }} setView={setView}/><PageIntro kicker={`${course.title} · Module ${module}`} title={lesson} text="Everything you need for today's English lesson." accent={course.accent}/><section className="content-section"><div className="section-heading"><div><span className="eyebrow">Lesson materials</span><h2>Let's learn</h2></div><p>Open a section to get started</p></div><div className="materials-grid">{materialCards.map(item => <button className="material-card" key={item.title} style={{ '--material': item.color } as React.CSSProperties}><span className="material-icon">{item.icon}</span><span><strong>{item.title}</strong><small>{item.text}</small></span><i><ArrowIcon/></i></button>)}</div><div className="homework-card"><span className="homework-icon">✓</span><div><span className="eyebrow">After the lesson</span><h2>Homework</h2><p>Complete the tasks and practise what you learned today.</p></div><button>Open homework <ArrowIcon/></button></div></section></main>
}
function App() {
